...
def add_feature_interactions(df, cols, top_n=10):
    """Create interaction features between the most important columns."""
    # Select a subset of columns to avoid explosion of features
    selected_cols = cols[:min(top_n, len(cols))]
    
    # Ratios between features
    for i, col1 in enumerate(selected_cols):
        for col2 in selected_cols[i+1:]:
            # Avoid division by zero with a safe denominator
            df[f"ratio_{col1}_{col2}"] = df[col1] / (df[col2] + 1e-8)
            
            # Differences
            df[f"diff_{col1}_{col2}"] = df[col1] - df[col2]
            
            # Products can capture non-linear relationships
            df[f"prod_{col1}_{col2}"] = df[col1] * df[col2]
